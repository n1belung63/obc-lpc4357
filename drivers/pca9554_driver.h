...
int32_t Pca9554<addr,dir,pol,state>::Init() {
	...
	res = WriteRegister(REG_CONFIGURATION, dir);
	if(res) return res;
	
	res = WriteRegister(REG_POLARITY_INVERSION, pol);
	if(res) return res;
	
	res = WriteRegister(REG_OUTPUT_PORT, state);
	if(res) return res;
...
int32_t Pca9554<addr,dir,pol,state>::WriteRegister(uint8_t reg, uint8_t data) {
	...
	return i2c_.Write(addr_, (uint8_t[2]){ reg, data }, 2);
}
...
int32_t Pca9554<addr,dir,pol,state>::ReadRegister(uint8_t reg, uint8_t* data){
	...
	res = i2c_.Write(addr_, &reg, 1);
	if(res) return res;
	
	res = i2c_.Read(addr_, data, 1);
	if (res) return res;
...
int32_t Pca9554<addr,dir,pol,state>::PinSet(uint8_t pin) {
	...
	res = ReadRegister(REG_OUTPUT_PORT, &current_state);
	if(res) return res;

	current_state |= (0x01<<pin);
	res = WriteRegister(REG_OUTPUT_PORT, current_state);
	if(res) return res;
