...
private:
	...
	inline __attribute__((always_inline)) int32_t FindNewestTime(uint32_t* newest_time) {
		...
		uint8_t tme_code = static_cast<uint8_t>(sector_num);
		uint8_t sd_num = static_cast<uint8_t>(num);

		uint32_t page_to_write_left;
		if (ds_vars_arr_[sd_num][tme_code].page_to_write == TConfig::GetSectorPars(tme_code).tme_range_start) {
			page_to_write_left = TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1;
		} else {
			page_to_write_left = ds_vars_arr_[sd_num][tme_code].page_to_write - 1;
		}
		
		RAIIAllocatedBuffer<uint32_t,TPoolAllocatorPort> time_buf(TConfig::GetSectorPars(tme_code).tme_max_num_in_page * sizeof(uint32_t));
		...
		*newest_time = FindMaxTimeInTimes<num,sector_num>(time_buf.data, ZERO_TIME);
		...
		err_code_ = GetAllTimesInPage<num,sector_num>(ds_vars_arr_[sd_num][tme_code].page_to_write, time_buf.data);
		if (err_code_ != ERROR_CODE_OK) {
			return err_code_;
		}
	...
	inline __attribute__((always_inline)) int32_t FindOldestTime(uint32_t* oldest_time) {
		...
		if (ds_vars_arr_[sd_num][tme_code].page_to_write == TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1) {
			page_to_write_right = TConfig::GetSectorPars(tme_code).tme_range_start;
		} else {
			page_to_write_right = ds_vars_arr_[sd_num][tme_code].page_to_write + 1;
		}

		RAIIAllocatedBuffer<uint32_t,TPoolAllocatorPort> time_buf(TConfig::GetSectorPars(tme_code).tme_max_num_in_page * sizeof(uint32_t));
				
		err_code_ = GetAllTimesInPage<num,sector_num>(TConfig::GetSectorPars(tme_code).tme_range_start, time_buf.data);
		if (err_code_ != ERROR_CODE_OK) {
			return err_code_;
		}		
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::InitSector() {
	...
	err_code_ = FindPageToWrite<num,sector_num>(&ds_vars_arr_[sd_num][tme_code].page_to_write);
	if (err_code_ != ERROR_CODE_OK) {
		return err_code_;
	}
	...
	ds_vars_arr_[sd_num][tme_code].tme_num_in_page = 0;
	if (IsThisTimeInArray<num,sector_num>(time_buf.data, NULL_TIME)) {
		for(uint8_t i = 0; i < TConfig::GetSectorPars(tme_code).tme_max_num_in_page; i++) {
			if (time_buf.data[i] == NULL_TIME) {
				ds_vars_arr_[sd_num][tme_code].tme_num_in_page = i;
				break;
			}
		}
	}
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::AddTmeToSd(uint8_t *tme) {
	...
	if (ds_vars_arr_[sd_num][tme_code].tme_num_in_page == 0) {
		memset((void*)buf.data, FREE_BYTE, BUFFER_SIZE);
	}
	
	if (ds_vars_arr_[sd_num][tme_code].tme_num_in_page > 0 && ds_vars_arr_[sd_num][tme_code].tme_num_in_page <= TConfig::GetSectorPars(tme_code).tme_max_num_in_page) {
		err_code_ = ReadPageWithAddrToBuf<num>(ds_vars_arr_[sd_num][tme_code].page_to_write, buf.data);
		if (err_code_ != ERROR_CODE_OK) {
			return err_code_;
		}
	}
	
	memcpy((void*)&buf.data[ds_vars_arr_[sd_num][tme_code].tme_num_in_page * TConfig::GetSectorPars(tme_code).tme_size], (void*)tme, TConfig::GetSectorPars(tme_code).tme_size);
	
	err_code_ = WriteBufToPageWithAddr<num>(ds_vars_arr_[sd_num][tme_code].page_to_write, buf.data);
	if (err_code_ != ERROR_CODE_OK) {
		return err_code_;
	}
			
	if (++ds_vars_arr_[sd_num][tme_code].tme_num_in_page == TConfig::GetSectorPars(tme_code).tme_max_num_in_page) {
		ds_vars_arr_[sd_num][tme_code].tme_num_in_page = 0;
		ds_vars_arr_[sd_num][tme_code].page_to_write++;
		
		if (ds_vars_arr_[sd_num][tme_code].page_to_write == TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length) {
			ds_vars_arr_[sd_num][tme_code].page_to_write = TConfig::GetSectorPars(tme_code).tme_range_start;
		}
	}
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::ReadTmeByTime(uint32_t tme_time, uint8_t *tme_data) {
	...
	memcpy((void*)tme_data, (void*)&buf.data[tme_num_in_page_ * TConfig::GetSectorPars(tme_code).tme_size], TConfig::GetSectorPars(tme_code).tme_size);
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::FindPageToRead(uint32_t tme_time, uint32_t *page_to_read) {
	...
	uint32_t tme_range_start = TConfig::GetSectorPars(tme_code).tme_range_start;
	uint32_t tme_range_end = tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1;
	
	uint32_t page_to_write = ds_vars_arr_[sd_num][tme_code].page_to_write;
	...
	if (form == form_2) {
		...
		time_end = time_buf.data[ TConfig::GetSectorPars(tme_code).tme_max_num_in_page - 1 ];
	}
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::FindPageToWrite(uint32_t *page_to_write) {
	...
	uint32_t L = TConfig::GetSectorPars(tme_code).tme_range_start;
	uint32_t R = TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1;
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::GetAllTimesInPage(uint32_t addr, uint32_t *times) {	
	...
	assert(TConfig::GetSectorPars(tme_code).tme_range_start <= addr && 
		addr <= TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1);
	...
	for(uint16_t i = 0; i < TConfig::GetSectorPars(tme_code).tme_max_num_in_page; ++i) {
		memcpy((void*)&times[i], (void*)&buf.data[i * TConfig::GetSectorPars(tme_code).tme_size], sizeof(uint32_t));
	}
...
uint32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::FindNumInBufByTime(uint8_t* buf, uint32_t tme_time) {
	...
	for(uint8_t i = 0; i < TConfig::GetSectorPars(tme_code).tme_max_num_in_page; i++) {
		if (*reinterpret_cast<uint32_t*>(&buf[ i * TConfig::GetSectorPars(tme_code).tme_size ]) == tme_time) {
			return i;
		}
	}
...
int32_t DataStorage<TBoard, TPoolAllocatorPort, TConfig>::ReadTmeBunch(uint32_t tme_time, uint32_t tme_step, uint32_t tme_qty, uint8_t *tmes_data) {
	...
	memset(tmes_data, FREE_BYTE, tme_qty * TConfig::GetSectorPars(tme_code).tme_size);

	uint32_t newest_time, oldest_time;
	...
	uint32_t tme_num_in_page_first, page_to_read_first, page_to_read_prev, tme_time_read;

	for(uint32_t i = 0; i < tme_qty; i++) {
		switch (alg) {
			case read_tme_by_addr_and_num: {
				tme_num_in_page_ = tme_num_in_page_first % (uint32_t)TConfig::GetSectorPars(tme_code).tme_max_num_in_page;
				page_to_read_ = page_to_read_first + tme_num_in_page_first / (uint32_t)TConfig::GetSectorPars(tme_code).tme_max_num_in_page;

				while (page_to_read_ > TConfig::GetSectorPars(tme_code).tme_range_start + TConfig::GetSectorPars(tme_code).tme_range_length - 1) {
					page_to_read_ -= TConfig::GetSectorPars(tme_code).tme_range_length;
				}
				...
				if (*reinterpret_cast<uint32_t*>(&buf.data[ tme_num_in_page_ * TConfig::GetSectorPars(tme_code).tme_size ]) == tme_time) {
					memcpy((void*)&tmes_data[ i * TConfig::GetSectorPars(tme_code).tme_size ], (void*)&buf.data[ tme_num_in_page_ * TConfig::GetSectorPars(tme_code).tme_size ], TConfig::GetSectorPars(tme_code).tme_size);
			...
			case read_tme_by_time: {				
				...
				memcpy(
					(void*)&tmes_data[ i * TConfig::GetSectorPars(tme_code).tme_size ],
					(void*)&buf.data[ tme_num_in_page_ * TConfig::GetSectorPars(tme_code).tme_size ], 
					TConfig::GetSectorPars(tme_code).tme_size
				);
